...
| D33 | **Live frames pass through an optional, user-tuned jitter buffer (`web/jitter-buffer.js`), off by default.** A real PS4 session showed the PS4-to-laptop UDP hop delivering nothing for ~100 ms about twice a second, then a burst (measured with the bridge's `--log-gaps`; the browser and the bridge's own send were fine, and `TCP_NODELAY` on the WebSocket changed nothing), which the page showed as a freeze then a jump. The buffer holds each frame until `frame.t` plus the least-delayed recent path plus a delay chosen on a 0-300 ms slider in the setup panel's Live tab, so a burst is replayed evenly. It is a slider, not a fixed number, because the smallest delay that hides a given network's stalls varies and costs display latency; a readout of frames that still arrived too late (counted even at 0) is what the user tunes against. The lap tracker uses the game's own `t`, so lap and sector times are unaffected; only the display lags. It cannot help when the bridge's `t` falls back to arrival time (game clock not trusted), since then the jitter is in `t` itself. | `web/jitter-buffer.js`, `web/app.js`, `web/index.html`, `bridge/bridge.py` (`GapLog`) |
