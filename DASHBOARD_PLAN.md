...
- The frame format gains suggested gear, rev-warning and rev-limiter rpm, and total laps. `paused`, `loading` and `onTrack` come from the flags ([Known issue 6](PROJECT_CONTEXT.md)). The bridge stamps each frame with its own receive time; revisit using the packet id once real packets show how it behaves.
