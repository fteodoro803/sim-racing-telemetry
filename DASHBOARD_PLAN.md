...
17. **Palette drawer now pushes the grid instead of overlapping it.** The user found the drawer covering widgets on the right edge clunky. `#grid` and `#palette` moved into a shared flex-row `.workspace` wrapper in `index.html` (a new flex-column item between the topbar and the rest); `.palette` in `web/style.css` dropped its `position: absolute` in favour of `flex: 0 0 min(300u, 80vw)`, so opening it now shrinks `.grid` (already relative/percentage-sized, so widgets reflow cleanly) rather than floating on top. `#emptyState` and `#overlay` moved alongside `#grid`/`#palette` as `.workspace` children (not inside `#grid` itself, since `renderGrid` replaces all of `#grid`'s children on every rebuild) - centering them on `.workspace` instead of the old `.app`/viewport is incidentally more accurate, since it now excludes the topbar. On phone portrait, where there's no room to push, `.palette` is overridden back to a full-screen `position: absolute` overlay, same as before. Checked in a browser at 1180×820 (palette opens/closes, grid reflows, widgets still readable) and 390×844 (palette still full-screens). `npm test` (76) unaffected (pure HTML/CSS).
18. **Palette is now always open while editing, not a toggle.** Following on from log 17, the user wanted the drawer visible for the whole of edit mode rather than needing an "Add widget" press first. Removed the `paletteBtn` button from `editRight` and `togglePalette` from `web/app.js`; `enterEditMode` now unhides `#palette` directly, and `doneEditing` still hides it on the way out (unchanged). The empty state's now-redundant "Add widgets" button (it only ever unhid the same drawer) was removed along with it, and its text points at "the palette on the right" instead. Checked in a browser: entering edit mode opens the drawer immediately with no extra click, Done hides it and returns the grid to full width. `npm test` (76) unaffected (no logic touched).
19. **Palette picks a variant before placing, not after.** The user found it backwards that a multi-variant widget (RPM + Gear, Pedals) landed on the grid with the default shape and only then let you change it via the post-placement `<select>` from D27. A palette entry with more than one `WIDGET_META` variant now expands inline on click into a row of variant buttons (`web/app.js`'s `buildPaletteList`, tracking which entry is open in module state `expandedPaletteWidget`) instead of adding immediately; picking one calls `editor.addWidget(widgetId, variantId)` (`web/edit.js`'s `addWidget` gained the optional `variant` parameter, falling back to `defaultVariant` when omitted, as it still does for single-variant widgets clicked directly). The post-placement `<select>` in the edit-mode chrome is unchanged - it's still there to change your mind afterwards. Checked in a browser: clicking "RPM + Gear" opens Stacked/Column/Ring inline instead of adding the widget; picking "Column" places it already showing Column in both the grid and its chrome's select; a no-variant widget (Speed) still adds on the first click. `npm test` (76) unaffected (no logic touched).
    - **Follow-up, same session**: text-only variant buttons weren't enough to tell shapes apart. First attempt drew a hand-built sketch per variant (a purpose-built mini diagram, avoiding the real widget's CSS to sidestep D25's hard pixel-minimum sizes at icon scale) - but the user pointed out the sketches didn't actually match the real widget (the Ring sketch was a closed pie slice; the real thing is an open 270° arc), which defeats the point of previewing before you pick. Replaced it with `buildVariantPreview` (`web/app.js`): builds and draws the actual widget via its own `build`/`update` from `widgets.js`, same as `renderGrid` does, at a smaller fixed size (`.vi-preview`, `calc(96 * var(--u))` tall) - `.widget` is already a CSS size container so D25's fluid clamps scale down cleanly without overflowing at that size. `frame` is faked from a static `PREVIEW_FRAME` (plausible gear/rpm/pedal values); `tracker`/`live` aren't, so this only works for widgets whose `update` reads `frame` alone - true of both `rpmGear` and `pedals` today, not enforced further. Checked in a browser: Stacked/Column/Ring and Vertical/Horizontal previews are pixel-for-pixel the real widget, just smaller; placing one still picks up live data immediately afterward; no console errors. `npm test` (76) unaffected (no logic touched).
20. **The Timing widgets gained explicit variants** ([D29](DECISIONS.md)), from a "Timing widgets" section added to `design/Widget Responsive Behavior.dc.html`. Seven widgets gained a `variants` list (D27's mechanism, unchanged): Delta (`minimal`/`gauge`), Sectors (`segments`/`list`/`chips`/`fill`), Delta Chart (`area`/`areaReadout`), Last Lap (`stacked`/`row`/`dual`/`trend`), Best Lap (`minimal`/`lapNumber`/`sectorSplit`/`wideStrip`), Predicted (`stacked`/`basisNote`/`range`/`compact`) and Lap Table (`table`/`compactList`/`rowCards`/`spreadsheet`); Current Lap wasn't touched, since the mockup only gave it one design and it already matched what was built. Each widget follows the same pattern as `rpmGear`/`pedals`: `build()` constructs every variant's DOM once, `update()` populates all of them every cycle, and `web/style.css`'s `[data-variant=…]` rules pick which one shows. New data needed: `LapTracker#worstLap` (`web/timing.js`, mirrors `bestLap`, for Predicted's `range` variant plotting the estimate between the session's best and worst lap) and an optional `fill` on a signed series in `charts.js`'s `drawChart`/`strokeSigned` (split into `signedRuns` + a new `fillSigned`, for Delta Chart's `area`/`areaReadout`), unused by the Speed chart. None of the new variants repeat the widget's own title text inside the body, since the card header already shows it - the standalone mockup previews do, so the built widgets read a little sparser than the mockups by design. Best Lap's `minimal` variant is the only one keeping the theoretical-best sub-line ([D29](DECISIONS.md)); none of the new mockup options had room for it. `WIDGET_META`'s `minW`/`minH` weren't changed - D25's fluid cqh/cqw clamps already shrink a variant's content gracefully at whatever size the widget's existing floor allows. Preset layouts (`PRESET_LAYOUTS`) now name each of these widgets' default variant explicitly, matching the existing `rpmGear`/`pedals` entries; `tests/layout.test.mjs`'s preset-consistency check requires it once a widget has `variants`. Added `worstLap` coverage to `tests/timing.test.mjs`. `npm test` (77) passes.
21. **The Driving widgets were right-sized against a re-issued design doc** ([D30](DECISIONS.md)). The user tightened `design/Widget Responsive Behavior.dc.html`'s Tyres/RPM+Gear/Speed/Pedals boxes to their real grid-cell sizes and padding; an audit (registry sizes read from `WIDGET_META`, fill measured live in a browser rather than guessed from CSS) found `rpmGear`'s add-size was 3×2 against a 2×2 mockup (the user's "Ring should be 2×2" report, which turned out to apply to all three shapes), `tyres` was 3×3 against 2×2, `pedals`' add-size (2×3) matched neither shape, and `speed` had no shape choice despite the doc now drawing two. Fixed in `web/layout.js`: `rpmGear` add-size → 2×2; `tyres` min/add-size → 2×2; `pedals` add-size → 2×2 (matches `vertical`, the default); `speed` gained an `inline`/`stacked` `variants` pair (D27 pattern, `inline` default - same DOM, just `.speed-row`'s flex direction switches per `[data-variant]`), with `minH` dropped to 1 so `inline`'s true 2×1 floor is reachable (same reasoning as Pedals' `minH` drop in log 13). `PRESET_LAYOUTS`'s two `speed` entries now name `variant: 'inline'` explicitly, required once a widget has `variants` (`tests/layout.test.mjs`'s preset-consistency check). In `web/style.css`: `.pedal-track`'s width ceiling raised (`min(14cqh, 55cqw, 40px)` → `min(20cqh, 90cqw, 90px)`) - measured live at the true 2×2 floor, the track went from 54% to 77% of its column's width; `.tyre`'s width ceiling loosened as headroom (`170px`/`27cqw`/`35cqh` → `260px`/`35cqw`/`45cqh`) for when Tyres is resized above its floor - not broken at the floor itself (measured ~97% fill there already), so lower priority. Checked in a browser: all four widgets land at their corrected size when added from the palette, RPM+Gear's Ring variant renders as a proper square gauge at 2×2, Speed's variant select switches between inline and stacked. Surfaced but did not fix [BUG-3](BUGS.md) (Tyres clips on the phone single-column layout - a `container-type: size`/`grid-auto-rows: auto` interaction unrelated to these desktop-grid sizes). Right-sizing (registry size against the mockup's tag, fill checked live) is now a standing convention, folded into `CLAUDE.md`. `npm test` (77) passes.

22. **New widget: Pedal Trace** ([D31](DECISIONS.md)), from a "Pedal Trace" section added to `design/Widget Responsive Behavior.dc.html`. Throttle and brake, both plotted as positive 0-100% areas over a rolling last-few-seconds window (not the whole lap, which would blur the detail the widget exists to show) so their overlap - trail-braking, or a mistake - is visible directly, unlike the signed delta-style charts. `web/charts.js` gained `drawPedalTrace` (draws the two filled series with `globalCompositeOperation: 'screen'`, so the overlap blends to a lighter colour with no separate polygon to compute) and `pushTraceSample` (a small pure function maintaining the rolling sample buffer, trimmed to the window and reset on a backwards clock jump - i.e. a new session; unit-tested in `tests/charts.test.mjs`, since the rest of `charts.js` isn't). Four shapes, chosen the D27 way: `filled` (default), `lines`, `compact` (bare, no axis/labels/fill - for a small slot), `gridlines` (`filled` plus a faint vertical line every second). Its trace window (3-10 seconds, in 1s steps) is a new kind of per-widget setting, distinct from a shape - a small numeric choice rather than a different look - so it reuses D27's variant mechanism rather than a new one: `WIDGET_META.pedalTrace.windows` (a plain number list, `[3, 4, ..., 10]`), `item.window` in the layout, a second `<select>` in edit-mode chrome (`web/edit.js`'s `setWindow`/`buildEditChrome`, stacked below the shape select in `web/style.css`), and `defaultWindow` mirroring `defaultVariant`. Unlike every other multi-variant widget, the widget's own canvas-drawing JS (not just CSS) needs to know its current shape/window, so `update()` reads them off its own card's `dataset` (`body.closest('.widget')`, the same trick `deltaChart` already used for `.has-chart`) rather than through the shared `ctx`. Its sample buffer lives in the widget's own `refs`, not the lap tracker, since it's real-time rather than lap-relative and must survive lap boundaries and reference-lap changes untouched. Not placed in any built-in preset (palette-only, like Tyres before D30) - there wasn't an obvious slot to bump. The palette's variant-picker preview needed several synthetic samples fed in over time rather than one static frame (`app.js`'s `PEDAL_TRACE_PREVIEW`, a hand-picked throttle-lift-into-brake trace with a brief overlap), unlike every other widget's single `PREVIEW_FRAME`. Checked in a browser: all four shapes render correctly, the window select changes the visible history live, no console errors, and no horizontal overflow at 375×812. `npm test` (81, up from 77) passes.
