...
| Pedal Trace widget (rolling throttle/brake overlap chart) | Page | Implemented | `web/widgets.js`, `web/charts.js` | Last 3-10 seconds (1s steps), both pedals plotted 0-100% on one axis so overlap (trail-braking, or a mistake) shows directly, via screen-blended fills. Four shapes ([D27](DECISIONS.md)-style) plus a trace-window setting using the same mechanism ([D31](DECISIONS.md)). In the Everything and Driving presets |
...
| Live smoothing (jitter buffer) | Page | Implemented | `web/jitter-buffer.js`, `web/app.js` | Optional 0-300 ms slider in the Live tab (off by default) that replays bursty frames evenly; shows recent late-frame count to tune against (D33) |
...
| Customisable dashboard (Timing/Driving/Everything presets, edit mode, Custom layout) | Page | Implemented | `web/layout.js`, `web/dashboard-state.js`, `web/edit.js`, `web/app.js` | Drag, resize, add from a palette, remove; every preset persists edits to `localStorage`, including the built-ins ([D32](DECISIONS.md)) - each has "Reset to default" to drop the edit and fall back to its shipped arrangement, and an "edited" dot on its tab when it has one saved. iPad only, checked against the fake console. [D11](DECISIONS.md), [D23](DECISIONS.md), [D24](DECISIONS.md), [D32](DECISIONS.md); spec: [`DASHBOARD_PLAN.md`](DASHBOARD_PLAN.md) §8 |
