...
15. **Never run against a real PS4 and iPad.** Step 6 of [`DASHBOARD_PLAN.md`](DASHBOARD_PLAN.md). Done = the bridge run against the PS4 and the dashboard open on the iPad, with a real session recorded. Expect to correct: byte offsets, gear encoding (neutral and reverse), the meaning of the `onTrack` flag (if it is cleared in some modes, timing is held throughout and the top bar says so), whether the game updates `last_lap` when the lap counter changes, boost-free gear and rpm scaling, and the frame timestamp (the bridge uses receive time). Also check the http page from the iPad, and Auto-Lock.
