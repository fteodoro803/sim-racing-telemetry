...
2. **GT7 protocol details are only partly verified.** Confirmed on a real PS4 (type A, 2026-09-22): the heartbeat, the encryption and nonce constant, the packet size and rate, the offsets read so far, the pause and loading flags, that `last_lap` updates with the lap counter, and that the game's time value (`0x80`) is a usable clock. Still unconfirmed: packet types B, `~` and C (offsets and nonce constants), boost, laps in a race (`0x76`), gear encoding for reverse, the flag bits beyond 0–5, and what `0x84` and `0x86` hold. The detail, field by field, is in [`GT7_TELEMETRY.md`](GT7_TELEMETRY.md).
...
5. **Interrupted-lap handling is partly confirmed.** Frames flagged paused, loading or off-track are dropped and the lap invalidated ([D7](DECISIONS.md), [O11](DECISIONS.md)). A real session confirmed that the pause flag is set while the game is paused, that packets keep arriving with the game clock standing still, and that laps containing a pause are correctly excluded. Still to do: record a restart and a replay to see what those look like, fix the rules if needed, and turn a clean capture into test fixtures (a session with two or more unpaused laps, since the recorded one has only one valid lap). Also open: [O2](DECISIONS.md).
...
15. **The iPad path is untested; the PS4 path works.** The bridge has run against a real PS4 and the dashboard responded with live values from a Mac browser (2026-09-22): see [`GT7_TELEMETRY.md`](GT7_TELEMETRY.md) "Confirmed on a real PS4". That run found that arrival-time timestamps made the delta wobble and lap times drift from the game's, fixed by using the game's clock ([O14](DECISIONS.md), [BUG-2](BUGS.md)). Still to do: open the page from the iPad over the local network (with the Mac's firewall and the iPad's Auto-Lock in mind), run a full race for `laps_in_race`, and a session with several unpaused laps.
