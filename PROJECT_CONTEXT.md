...
1. **The bridge is only partly written.** The first slice, `bridge/capture.py`, sends the heartbeat, receives, decrypts, decodes type-A packets and records the raw packets; it passes its tests against a fake console but hasn't yet been run against a real PS4. Still to do: turn decoded packets into normalised frames, forward them over a WebSocket, serve the `web/` folder over http on the local network ([D16](DECISIONS.md)), and handle the other packet types, with one decoder module per game so others can be added. Language: Python ([D15](DECISIONS.md)).
