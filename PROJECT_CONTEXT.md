...
16. ~~**RPM and Gear are still two separate widgets.**~~ **Resolved** ([D28](DECISIONS.md); log 16 in [`DASHBOARD_PLAN.md`](DASHBOARD_PLAN.md)). Merged into one `rpmGear` widget with three variants (stacked, column, ring); the standalone `rpm`/`gear` entries are gone from the registry, palette and both built-in presets. Untested on real data (Known issue 15). A saved Custom layout that still names the old `gear` or `rpm` widget ids will silently drop those cards on next load (`renderGrid` already skips unknown widget ids) - no migration was written to convert them to `rpmGear`, since Custom didn't exist with those ids in it until now.
