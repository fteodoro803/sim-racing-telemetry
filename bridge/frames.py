...
def to_frame(decoded, t_ms):
    ...
    `loading` and `onTrack`, which the lap tracker uses to hold timing. `totalLaps` is left out for
    now because its offset in the packet is unconfirmed. `gear` is -1 in reverse, inferred per
    `_is_reversing` since GT7 doesn't signal it directly.
